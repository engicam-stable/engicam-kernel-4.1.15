...
/*MP:add*/	unsigned int x_max;
/*MP:add*/	unsigned int y_max;
};
...
static irqreturn_t edt_ft5x06_ts_isr(int irq, void *dev_id)
{
	...
	for (i = 0; i < MAX_SUPPORT_POINTS; i++) {
		...
		input_report_abs(tsdata->input, ABS_MT_POSITION_Y, tsdata->y_max - y);
	}
...
printk("MP: %s %d\n",__FUNCTION__,__LINE__);
	...
	/*MP:add*/	tsdata->x_max = 800;//pdata->x_max;
	/*MP:add*/	tsdata->y_max = 480;//pdata->y_max;
		if (error) {
			dev_err(&client->dev,
...
//	dev_dbg(&client->dev,
	printk(	"Model \"%s\", Rev. \"%s\", %dx%d sensors\n",
...
/*MP:era	input_set_abs_params(input, ABS_X, 0, tsdata->num_x * 64 - 1, 0, 0);
